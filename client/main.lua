...
Citizen.CreateThread(function()
	...
	while playerGroup == nil do
		ESX.TriggerServerCallback('VorkA-PersonalMenu:Admin_getUsergroup', function(group) playerGroup = group end)
		Citizen.Wait(0)
	end
...
RegisterNetEvent("VorkA-PersonalMenu:Weapon_addAmmoToPedC")
AddEventHandler("VorkA-PersonalMenu:Weapon_addAmmoToPedC", function(value, quantity)
...
RegisterNetEvent('VorkA-PersonalMenu:Admin_BringC')
AddEventHandler('VorkA-PersonalMenu:Admin_BringC', function(plyPedCoords)
	SetEntityCoords(plyPed, plyPedCoords)
end)

-- GOTO JOUEUR
function admin_tp_toplayer()
	local plyId = KeyboardInput("VorkA_BOX_ID", _U('dialogbox_playerid'), "", 8)
...
function admin_tp_playertome()
	...
	if plyId ~= nil then
		...
		if type(plyId) == 'number' then
			...
			TriggerServerEvent('VorkA-PersonalMenu:Admin_BringS', plyId, plyPedCoords)
		end
	end
end
...
function admin_tp_pos()
	local pos = KeyboardInput("VorkA_BOX_XYZ", _U('dialogbox_xyz'), "", 50)
...
function admin_vehicle_spawn()
	local vehicleName = KeyboardInput("VorkA_BOX_VEHICLE_NAME", _U('dialogbox_vehiclespawner'), "", 50)
...
function admin_give_money()
	local amount = KeyboardInput("VorkA_BOX_AMOUNT", _U('dialogbox_amount'), "", 8)

	if amount ~= nil then
		amount = tonumber(amount)
		
		if type(amount) == 'number' then
			TriggerServerEvent('VorkA-PersonalMenu:Admin_giveCash', amount)
...
function admin_give_bank()
	...
	if amount ~= nil then
		...
		if type(amount) == 'number' then
			TriggerServerEvent('VorkA-PersonalMenu:Admin_giveBank', amount)
			TriggerServerEvent("scrambler:AdminGiveArgent", amount)
		end
	end
end
...
function admin_give_dirty()
	...
	if amount ~= nil then
		...
		if type(amount) == 'number' then
			TriggerServerEvent('VorkA-PersonalMenu:Admin_giveDirtyMoney', amount)
			TriggerServerEvent("scrambler:AdminGiveArgent", amount)
		end
	end
end
				...
				if item == wepItem[value] then
					weaponItemMenu.OnItemSelect = function(sender, item, index)
						...
						elseif item == giveMunItem then
							local quantity = KeyboardInput("VorkA_BOX_AMMO_AMOUNT", _U('dialogbox_amount_ammo'), "", 8)
										...
										if ammo > 0 then
											if post == true then
												if quantity <= ammo and quantity >= 0 then
													...
													TriggerServerEvent('VorkA-PersonalMenu:Weapon_addAmmoToPedS', GetPlayerServerId(personalmenu.closestPlayer), value, quantity)
...
function AddMenuWalletMenu(menu)
	...
	walletmenu.SubMenu.OnListSelect = function(sender, item, index)
		if index == 1 then
			local quantity = KeyboardInput("VorkA_BOX_AMOUNT", _U('dialogbox_amount'), "", 8)
...
function AddMenuFacturesMenu(menu)
	...
	ESX.TriggerServerCallback('VorkA-PersonalMenu:Bill_getBills', function(bills)
		for i = 1, #bills, 1 do
		...
		if item == recruterItem then
			if ESX.PlayerData.job.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('VorkA-PersonalMenu:Boss_recruterplayer', GetPlayerServerId(personalmenu.closestPlayer), ESX.PlayerData.job.name, 0)
				end
			else
		...
		elseif item == virerItem then
			if ESX.PlayerData.job.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('VorkA-PersonalMenu:Boss_virerplayer', GetPlayerServerId(personalmenu.closestPlayer))
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
		elseif item == promouvoirItem then
			if ESX.PlayerData.job.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('VorkA-PersonalMenu:Boss_promouvoirplayer', GetPlayerServerId(personalmenu.closestPlayer))
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
		elseif item == destituerItem then
			if ESX.PlayerData.job.grade_name == 'boss' then
				...
				if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
					...
					TriggerServerEvent('VorkA-PersonalMenu:Boss_destituerplayer', GetPlayerServerId(personalmenu.closestPlayer))
				end
			else
				ESX.ShowNotification(_U('missing_rights'))
			end
